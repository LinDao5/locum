...
class Config extends AbstractForm
{
    ...
    public function initGeneral()
    {
        ...
        $site_currency = new Element\Text('email_currency');
        $site_currency->setLabel('Email Currency Symbol')
            ->setLabelAttributes(
                array(
                    'class' => 'required control-label col-lg-2'
                )
            )
            ->setAttribute('id', 'email_currency')
            ->setAttribute('class', 'form-control');
        ...
        $searchFieldset->setLabel('Locum Search Setting');
    ...
    public function initEmailConfig()
    {
        ...
        /* marketing invitation email manager */
        $emailFieldsetMarketing = new Fieldset('email_marketing');
        $emailFieldsetMarketing->setLabel('Marketing Invitation Email Manager');       

        $marketingMailIds = new Element\Textarea('user_marketing_mail_ids');
        $marketingMailIds->setLabel('Marketing Emails')
            ->setLabelAttributes( 
                array(
                    'class' => 'required control-label col-lg-2'
                )
            )
            ->setAttribute('id', 'marketing_mail_ids')
            ->setAttribute('class', 'form-control')
            ->setAttribute('placeholder', 'Enter emails like "locumkit1@gmail.com;locumkit2@gmail.com"')
            ->setAttribute('rows', '5');
        $emailFieldsetMarketing->add($marketingMailIds);

        $marketingMailSubject = new Element\Text('user_marketing_mail_subject');
        $marketingMailSubject->setLabel('Subject')
            ->setLabelAttributes(
                array(
                    'class' => 'required control-label col-lg-2'
                )
            )
            ->setAttribute('id', 'marketing_mail_subject')
            ->setAttribute('class', 'form-control');
        $emailFieldsetMarketing->add($marketingMailSubject);

        $marketingMailMessage = new Element\Textarea('user_marketing_mail_message');
        $marketingMailMessage->setLabel('Message')
            ->setLabelAttributes( 
                array(
                    'class' => 'required control-label col-lg-2',
                )
            )
            ->setAttribute('id', 'marketing_mail_message')
            ->setAttribute('class', 'form-control')
            ->setAttribute('rows', '10');
        $emailFieldsetMarketing->add($marketingMailMessage);
        $this->add($emailFieldsetMarketing);

        return $this;
    }

    /* Init notification config value */
    public function initNotificationConfig(){
        $notificationFieldset = new Fieldset('notification_setting');
        $notificationFieldset->setLabel('Notification time manager');
        $hr = 1;        
        while($hr <= 24 ){
            $hrArray[sprintf('%02d', $hr)] =  sprintf('%02d', $hr);
            $hr++;
        }

        $attebdJobNotification = new Element\Select('attend_job_notification');        
        $attebdJobNotification->setLabel('Attend Job Notification Time (in 24 hour format)')
            ->setValueOptions($hrArray)
            ->setLabelAttributes(
                array(
                    'class' => 'required control-label col-lg-2'
                )
            )
            ->setAttribute('id', 'attend_job_notification')
            ->setAttribute('class', 'form-control');
        $notificationFieldset->add($attebdJobNotification); 
        $this->getInputFilter()->add(
            array(
                'name' => 'attend_job_notification',
                'required' => true,
                'validators' => array(
                    array('name' => 'not_empty'),
                ),
        ...
        $expensesJobNotification = new Element\Select('expenses_job_notification');        
        $expensesJobNotification->setLabel('Expenses Job Notification Time (in 24 hour format)')
            ->setValueOptions($hrArray)
            ->setLabelAttributes(
                array(
                    'class' => 'required control-label col-lg-2'
            ...
            ->setAttribute('id', 'expenses_job_notification')
            ->setAttribute('class', 'form-control');
        $notificationFieldset->add($expensesJobNotification); 
        $this->getInputFilter()->add(
            array(
                'name' => 'expenses_job_notification',
                'required' => true,
                'validators' => array(
                    array('name' => 'not_empty'),
                ),
        ...
        $feedbackJobNotification = new Element\Select('feedback_job_notification');        
        $feedbackJobNotification->setLabel('Feedback Job Notification Time (in 24 hour format)')
            ->setValueOptions($hrArray)
            ->setLabelAttributes(
                array(
                    'class' => 'required control-label col-lg-2'
            ...
            ->setAttribute('id', 'feedback_job_notification')
            ->setAttribute('class', 'form-control');
        $notificationFieldset->add($feedbackJobNotification); 
        $this->getInputFilter()->add(
            array(
                'name' => 'feedback_job_notification',
                'required' => true,
                'validators' => array(
                    array('name' => 'not_empty'),
                ),
