	...
	*/
	...
	{			
        ...
        public function sendOnDayNotificationToFreelancer($jobId,$jobFid,$jobEid,$yesBtnLink,$adapter)
        {
	        ...
	        $massageFre = $mail_css.'
	        	<div style="padding: 25px 50px 30px; border-right: 2px solid #000; border-left: 2px solid #000;text-align: left;">
	        		<p>Hi <b>'.$freName.'</b>,</p>
    				<h3>Please can you confirm your arrival to work today</h3>
    				<p>'.$yesBtnLink.'</p>
        ...
        public function recievedFeedbackFreelancerNotification($feedbackId,$feedbackArray,$adapter)
        {
	        ...
	        $massageFre .= '<p>&nbsp;</p>';
	        $massageFre .= '<p><b>Average Star Rate: '.$averageRate.'</b></p>';
	        $massageFre .= '<p>&nbsp;</p>';
	        $massageFre .= '<p>If you feel this feedback is not a true reflection of your performance then please <a href="'.$serverUrl().'/feedback-dispute?feedbackId='.$encypt->encryptIt($feedbackId).'&u='.$encypt->encryptIt($freId).'">click here</a>, so we a LocumKit can look into this. </p>';
	      	$massageFre .= '<p>If you are happy with this then we shall  publish this feedback against your profile in the next 48 hours.</p>';
	        $massageFre .= '</div>';
	        $massageFre .= $footer;
        ...
        public function recievedFeedbackEmployerNotification($feedbackId,$feedbackArray,$adapter)
        {
	        ...
	        $massageEmp .= '<p>&nbsp;</p>';
	        $massageEmp .= '<p><b>Average Star Rate: '.$averageRate.'</b></p>';
	        $massageEmp .= '<p>&nbsp;</p>';
	        $massageEmp .= '<p>If you feel this feedback is not a true reflection of your performance then please <a href="'.$serverUrl().'/feedback-dispute?feedbackId='.$encypt->encryptIt($feedbackId).'&u='.$encypt->encryptIt($empId).'">click here</a>, so we a LocumKit can look into this. </p>';
	      	$massageEmp .= '<p>If you are happy with this then we shall  publish this feedback against your profile in the next 48 hours.</p>';
	        $massageEmp .= '</div>';
	        $massageEmp .= $footer;
	        $currentDate = date('Y-m-d');
	        /* Mail Send to freelancer */
...
Once the cancellation has been received, we will contact you to advise that it has been dealt with. If the cancellation is short notice then please contact the store directly as well as emailing LocumKit.
        ...
        public function sendExpireFreezeNotification($job_id,$f_id,$expired_note_type,$adapter,$link)
        {
	        ...
	        if ($expired_note_type == 2) {
	        	...
	        	$mail_title = '<p>A job posting you were previously invited to has now become available again. [Ref?] The job specifics are detailed below. To accept this offer, please click on accept button. </p> ';
	        }
			$user_info ='
					<h3 style="text-align:left;"> '.$mail_title.' </h3>
					'.$job_info.'<br/>
					'.$link.'
				</div>'.$footer.'</body></html>';
	        $massageFre = $mail_css.'
 ...
 WHERE p_uid = '$puId'";	
        ...
        public function sendDisputeSubmitNotification($id,$fre_id,$emp_id,$job_id,$user_type,$avg_rate,$adapter)
        {
		    ...
		    if ($user_type == 2) {
		        ...
		        $disputeFreMsg .= '<p><b style="text-transform: capitalize;">'.$empName.'</b> submit dispute on feedback you submitted on job <b>#'.$job_id.'</b>. </p>';
		        
		        $disputeFreMsg .= '<p>Please contact admin as soon as possible to take appropriate action on it.</p>';
		        ...
		        $disputeEmpMsg .= '<p>You have successfully submit dispute on feedback that submitted by <b style="text-transform: capitalize;">'.$freName.'</b> on job <b>#'.$job_id.'</b>. </p>';
		        
		        $disputeEmpMsg .= '<p>Please contact admin as soon as possible to take appropriate action on it.</p>';
		        ...
		        $disputeEmpMsg .= '<div style="padding: 25px 50px 30px; border-right: 2px solid #000; border-left: 2px solid #000;text-align: left; font-family: sans-serif;">
		        		<p>Hi <b style="text-transform: capitalize;">'.$empName.'</b>,</p>';
		        $disputeEmpMsg .= '<p><b style="text-transform: capitalize;">'.$freName.'</b> submit dispute on feedback you submitted on job <b>#'.$job_id.'</b>. </p>';
		        ...
		        $disputeFreMsg .= '<div style="padding: 25px 50px 30px; border-right: 2px solid #000; border-left: 2px solid #000;text-align: left; font-family: sans-serif;">
		        		<p>Hi <b style="text-transform: capitalize;">'.$freName.'</b>,</p>';
		        $disputeFreMsg .= '<p>You have successfully submit dispute on feedback that submitted by <b style="text-transform: capitalize;">'.$empName.'</b> on job <b>#'.$job_id.'</b>. </p>';
