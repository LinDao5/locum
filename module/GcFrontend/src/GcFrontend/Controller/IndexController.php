...
class IndexController extends Action
{
    ...
    public function indexAction()
    {
        ...
        if (empty($document)) {
            ...
            $req_url_array = explode('/', $_SERVER['REQUEST_URI']);

            if($req_url_array[1] == 'blog' && isset($req_url_array[2])){
                $slugHelper = new FinanceHelper();
                $blogPost = $slugHelper->checkBlogSlugExist($req_url_array[2], 'blog');
                if (!empty($blogPost[0])) { 
                    $viewModel->setTemplate('view/fs-single-blog');
                    $viewModel->setTerminal(true);
                    $viewModel->postData = $blogPost[0];
                    return $viewModel;
                }                
            }

            if($req_url_array[1] == 'news' && isset($req_url_array[2])){
                $slugHelper = new FinanceHelper();
                $newsPost = $slugHelper->checkBlogSlugExist($req_url_array[2], 'news');
                if (!empty($newsPost[0])) {
                    $viewModel->setTemplate('view/fs-single-blog');
                    $viewModel->setTerminal(true);
                    $viewModel->postData = $newsPost[0];
                    return $viewModel;
                }                
            }
