...
{   
    ...
    public function getAllIncome($uid , $financialYear , $filter , $openinvoice = false , $cat = null , $isbank = null , $supplier = null)
    {
		...
		$financialYear_3 = $financialYear - 2 ;
            	$financeyear = "AND (YEAR(fin.job_date) <=  '$financialYear' AND  YEAR(fin.job_date) >=  '$financialYear_3')";}elseif($financialYear != null){
			$financeyear = "AND YEAR(fin.job_date) =  '$financialYear'";
		}
		if($openinvoice == true){
        ...
        $sqlAllincome ="SELECT f.trans_id ,f.trans_type  ,fin.* FROM finance f, finance_income_table fin WHERE fin.fre_id = '$uid' AND fin.id = f.trans_type_id AND f.trans_type = '1' $financeyear order by f.trans_id desc";	
    ...
    public function getAllExpense($uid , $financialYear , $filter , $cat = null , $isbank = null)
    {
		...
		if($financialYear != null && $filter == 'year'){
		 $financialYear_3 = $financialYear - 2 ;
            	$financeyear = "AND (YEAR(ex.job_date) <=  '$financialYear'  AND  YEAR(ex.job_date) >=  '$financialYear_3')";}elseif($financialYear !=null){
			$financeyear = "AND YEAR(ex.job_date) =  '$financialYear'";
		}if($cat != null){
			$financeyear .= " AND expense_type_Id = $cat";
		}if($isbank != null){
			$financeyear .= " AND bank = '1' ";
		}
        $sqlAllExpense="SELECT f.trans_id, f.trans_type ,ex.* FROM finance f, finance_expense_table ex WHERE ex.fre_id = '$uid' AND ex.id = f.trans_type_id AND f.trans_type = '2' $financeyear order by f.trans_id desc";
	...
	public function chart_getAllIncome($uid , $financialYear , $filterType , $category = null , $supplier = null)
    {
		...
		if($filterType == 'year'){
			...
			$sqlAllincome ="SELECT YEAR(job_date) as year, SUM(IF(bank = 1 , job_rate , 0)) AS bank_yes , SUM(IF(bank = 1 , 0 , job_rate)) AS bank_no FROM finance_income_table where fre_id = '$uid' and (YEAR(job_date) <=  '$financialYear'  AND YEAR(job_date) >=  '$financialYear_3')  $cat GROUP BY YEAR(job_date)";
		...
		public function getAllIncomeBymonth($uid , $financialYear ,$filtermonth , $category = null , $supplier = null )
    {
		...
		$sqlAllincome ="SELECT MONTHNAME(job_date) as month_nm , SUM(IF(bank = 1 , job_rate , 0)) AS bank_yes , SUM(IF(bank = 1 , 0 , job_rate)) AS bank_no FROM finance_income_table where fre_id = '$uid' and YEAR(job_date) =  '$financialYear' and MONTH(job_date) =  '$filtermonth ' $cat GROUP BY YEAR(job_date) , MONTH(job_date)";		
        $allincomeObj = $adapter->query($sqlAllincome, $adapter::QUERY_MODE_EXECUTE); 
	...
	public function chart_getAllExpense($uid , $financialYear , $filterType , $category = null )
    {
		...
		if($filterType == 'year'){
			...
			$sqlexpense ="SELECT YEAR(job_date) as year, SUM(IF(bank = 1 , cost , 0)) AS bank_yes , SUM(IF(bank = 1 , 0 , cost)) AS bank_no FROM finance_expense_table where fre_id = '$uid' and (YEAR(job_date) <=  '$financialYear'  AND YEAR(job_date) >=  '$financialYear_3') $cat GROUP BY YEAR(job_date)";
...
$AllMonth = $this->getMonthrangeByyear($financialYear , $uid);
	...
	public function getAllExpenseBymonth($uid , $financialYear ,$filtermonth ,$category = null  )
    {
		...
		$sqlAllExpense ="SELECT YEAR(job_date) as year, MONTHNAME(job_date) as month_nm , SUM(IF(bank = 1 , cost , 0)) AS bank_yes , SUM(IF(bank = 1 , 0 , cost)) AS bank_no FROM finance_expense_table where fre_id = '$uid' and YEAR(job_date) = '$financialYear' and MONTH(job_date) =  '$filtermonth' $cat  GROUP BY YEAR(job_date) , MONTH(job_date)";
	    $allExpenseObj = $adapter->query($sqlAllExpense, $adapter::QUERY_MODE_EXECUTE); 
...
public function getExpenseCategoryName($id = null)
{
	try{
		$dbConfig = new DbController();
        $adapter = $dbConfig->locumkitDbConfig();
		$select = "SELECT * FROM expense_type WHERE id = '$id'";
		$extypeObj = $adapter->query($select, $adapter::QUERY_MODE_EXECUTE); 
		$extype = $extypeObj->current();
    ...
    {
        $adapter = $this->getAdapter();	
			$sqlexpense ="SELECT YEAR(job_date) as year, expense_type_id , SUM(cost) AS cost FROM finance_expense_table where fre_id = '$uid' and YEAR(job_date) =  '$financialYear' GROUP BY YEAR(job_date) , expense_type_id";
			$allexpenseObj = $adapter->query($sqlexpense, $adapter::QUERY_MODE_EXECUTE); 
			$allexpense = $allexpenseObj->toArray();
    ...
    {
        $adapter = $this->getAdapter();	
			$sqlincome ="SELECT YEAR(job_date) as year, income_type , SUM(job_rate) AS job_rate FROM finance_income_table where fre_id = '$uid' and YEAR(job_date) =  '$financialYear' GROUP BY YEAR(job_date) , income_type";
    ...
    public function getAllIncomeByweek($uid , $financialYear ,$filterweekday = null , $category = null )
    {
        ...
        $sqlAllincome ="SELECT DAYOFWEEK(job_date) as dayno , DAYNAME(job_date) as day_nm , SUM(job_rate) AS bank_yes , SUM(IF(bank = 1 , 0 , job_rate)) AS bank_no FROM finance_income_table where income_type = '1' and fre_id = '$uid' and YEAR(job_date) = '$financialYear' $con GROUP BY YEAR(job_date) , DAYOFWEEK(job_date)";
        $allincomeObj = $adapter->query($sqlAllincome, $adapter::QUERY_MODE_EXECUTE);
    ...
    {
        $adapter = $this->getAdapter();
        $sqlincome ="SELECT YEAR(job_date) as year, location , SUM(job_rate) AS job_rate FROM finance_income_table where fre_id = '$uid' and YEAR(job_date) =  '$financialYear' GROUP BY YEAR(job_date) , location";
        $allincomeObj = $adapter->query($sqlincome, $adapter::QUERY_MODE_EXECUTE);
        $allincome = $allincomeObj->toArray();
        $dataX = $data1 = $data2 =  $cat_data = '';
        foreach ($allincome as $income){
   ...
   public function GetchartIncomeBYsupplier($uid , $financialYear)
    {
        $adapter = $this->getAdapter();
        $sqlincome ="SELECT YEAR(job_date) as year, supplier , SUM(job_rate) AS job_rate FROM finance_income_table where fre_id = '$uid' and YEAR(job_date) =  '$financialYear' GROUP BY YEAR(job_date) , supplier";
        $allincomeObj = $adapter->query($sqlincome, $adapter::QUERY_MODE_EXECUTE);
        $allincome = $allincomeObj->toArray();
        $dataX = $data1 = $data2 =  $cat_data = '';
        foreach ($allincome as $income){
...
'invoice_id' => $dt1['invoice_id'],
    ...
    public function getEmpFinanceCost($uid , $financialYear)
    {
        ...
        if($AllMonth != ''){
            foreach($AllMonth as $key => $Month1){
                $monthWise = $this->getEmpFinanceBymonth($uid ,$financialYear,$key,'cost');
                if($monthWise != ''){
                    $dataX[] = @$monthWise['month_nm'] ;
    ...
    public function getEmpFinanceJob($uid , $financialYear)
    {
        ...
        if($AllMonth != ''){
            foreach($AllMonth as $key => $Month1){
                $monthWise = $this->getEmpFinanceBymonth($uid ,$financialYear,$key,'job');
                if($monthWise != ''){
                    $dataX[] = @$monthWise['month_nm'] ;
    ...
    public function getEmpFinanceBymonth($uid , $financialYear ,$filtermonth, $filtertype)
    {
        $adapter = $this->getAdapter();
        if($filtertype == 'job'){
            $sqlAllExpense ="SELECT YEAR(job_date) as year, MONTHNAME(job_date) as month_nm , count(job_id) as job_id FROM finance_employer where emp_id = '$uid' and YEAR(job_date) = '$financialYear' and MONTH(job_date) =  '$filtermonth' GROUP BY YEAR(created_at) , MONTH(created_at)";
        }if($filtertype == 'cost'){
        $sqlAllExpense ="SELECT YEAR(job_date) as year, MONTHNAME(job_date) as month_nm , SUM(job_rate)AS job_rate  FROM finance_employer where emp_id = '$uid' and YEAR(job_date) = '$financialYear' and MONTH(job_date) =  '$filtermonth' GROUP BY YEAR(created_at) , MONTH(created_at)";
    }
        $allExpenseObj = $adapter->query($sqlAllExpense, $adapter::QUERY_MODE_EXECUTE);
        $allExpense = $allExpenseObj->current();
        if($allExpense){
  ...
  return $jobtype ;
    ...
    public function InsertIndustrynews($data)
    {
        ...
        if(!isset($data['id']) && $data['id'] == ''){
            $sqlGerRegisterDate="INSERT INTO `industry_news`(`title`, `description`, `image_path`,  `user_type`, `category_id`, `status`) VALUES ('".$data['title']."','".$data['description']."','".$data['image_path']."','".$data['user_type']."','".$data['category_id']."','".$data['status']."')";
            $adapter->query($sqlGerRegisterDate, $adapter::QUERY_MODE_EXECUTE);
            return $adapter->getDriver()->getLastGeneratedValue();
        }else{
            ...
            $sqlGerRegisterDate="UPDATE industry_news SET  title = '".$data['title']."' , description = '".$data['description']."' , user_type = '".$data['user_type']."' , category_id = '".$data['category_id']."' , status = '".$data['status']."' ". $u ."   WHERE id = '".$data['id']."'";
            $adapter->query($sqlGerRegisterDate, $adapter::QUERY_MODE_EXECUTE);
        }
    }
