...
class DocumentController extends AbstractController
{
    ...
    public function createAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!$documentForm->isValid()) {
                ...
                $parentId       = $documentForm->getValue('parent_id');                
    ...
    public function editAction()
    {
        ...
        if(isset($_GET['save']) && $_GET['save'] == 'team-info'){           
            $team_img_array = $_FILES["team_image"]; 
            $teamFormData = $this->getRequest()->getPost()->toArray();
            $this->saveTeamInfo($teamFormData,$team_img_array);
            $this->flashMessenger()->addSuccessMessage('Team has been saved');
            ...
            if ($this->getRequest()->isPost()) {
                ...
                $document->setParentId(
                    empty($documentVars['parent_id']) ?
                    null :
                    $documentVars['parent_id']
                );            
    ...
    public function saveTeamInfo($teamFormData,$team_img_array)
    {
        if (!empty($team_img_array)) {
            $target_dir = $_SERVER['DOCUMENT_ROOT'].'/public/media/team/';
            $img_url ='https://'.$_SERVER['SERVER_NAME'];
            $team_img_url_array = array();
            foreach ($team_img_array['name'] as $key => $team_img) { 
                if (!empty($team_img_array['name'])) {
                    $target_file = $target_dir . basename($team_img);
                    if (file_exists($target_file)) {
                        $rawBaseName = pathinfo($team_img, PATHINFO_FILENAME );
                        $extension = pathinfo($team_img, PATHINFO_EXTENSION );
                        $team_img = $rawBaseName .'-'. rand(0,999) . '.' . $extension;
                        $target_file = $target_dir . $team_img;
                    }
                    if(move_uploaded_file($team_img_array["tmp_name"][$key], $target_file)){
                        $team_img_url_array[$key] = '/public/media/team/'.$team_img;
                    }
                } 
            } 
        }        
        ...
        foreach ($teamFormData['team_name'] as $key => $teamData) {
            $team_id = $key+1;
            $sqlCheckIfTeamExist = "SELECT team_id FROM locumkit_team_info WHERE team_id = '$team_id'"; 
            $getTeamInfo = $adapter->query($sqlCheckIfTeamExist, $adapter::QUERY_MODE_EXECUTE);
            if (empty($getTeamInfo->current())) {
                $sqlInserteamInfo = "INSERT INTO locumkit_team_info (name,designation,image,position) VALUES ('".$teamData."','".$teamFormData['team_designation'][$key]."','".$team_img_url_array[$key]."','".$teamFormData['team_position'][$key]."')"; 
                $adapter->query($sqlInserteamInfo, $adapter::QUERY_MODE_EXECUTE);
            }else{
                if ($team_img_url_array[$key] != '') {
                    $sqlUpdateteamInfo = "UPDATE locumkit_team_info SET name = '".$teamData."',designation = '".$teamFormData['team_designation'][$key]."',image = '".$team_img_url_array[$key]."',position = '".$teamFormData['team_position'][$key]."' WHERE team_id = '$team_id'"; 
                }else{
                    $sqlUpdateteamInfo = "UPDATE locumkit_team_info SET name = '".$teamData."',designation = '".$teamFormData['team_designation'][$key]."',position = '".$teamFormData['team_position'][$key]."' WHERE team_id = '$team_id'"; 
                }
                
                $adapter->query($sqlUpdateteamInfo, $adapter::QUERY_MODE_EXECUTE);
            }
