...
class DocumentInformation extends AbstractForm
{
    ...
    public function init()
    {
        ...
        $parentOptions = array('' => 'Select Parent Page');
        foreach ($documentCollection->getAvailableDocuments() as $key => $documents) {
            $parentOptions[$documents['id']]= $documents['name'];
        }
        $parent->setLabel('Parent Page')
            ->setLabelAttributes(array('class' => 'required control-label'))
            ->setAttribute('id', 'parent_id')
            ->setAttribute('class', 'form-control')
            ->setValueOptions($parentOptions);
