	...
	Class FinanceController
	{
		...
		public function getLocumFinanceSummary($uid,$financialYear)
		{
			...
			$overallincome = $financeHelper->getIncomeByuser($uid,$financialYear);
        		...
        		if(!empty($overallincome) && !empty($overallexpence)){
        			$profit = $overallincome['job_rate'] - $overallexpence['cost'];
        		}elseif(!empty($overallincome) && empty($overallexpence)){
        			$profit = $overallincome['job_rate'];
        		}elseif(empty($overallincome)){
        			$profit = 0;
        		}
        		...
        		$finance_summary = array(
        			...
        			'total_tax' => $helper->formate_price($financeController->taxclaculation($profit,$fmon_usertype,date('Y'),$uid)),
...
$finance_chart['finance_year'] = $financeHelper->getMonthFinancialYear($uid,date('Y'));
